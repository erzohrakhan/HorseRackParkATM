...
public class Inventory {
    ...
    public Inventory() {
        denominationList = Arrays.asList(
    ...
    private void calculateTotalAmount() {
        totalAmount = denominationList.stream()
    ...
    public void dispense(int payoutAmount) throws HorseTrackException{
        if (hasSufficientFunds(payoutAmount)) {
            for (int i = denominationList.size() - 1; i >= 0; i--) {
                ...
                denominationList.set(i, new Denomination(value, denominationList.get(i).getCount() - count));
